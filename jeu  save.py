...
class Joueur:
    ...
    def move(self):
        ...
        #gravite
        if active_map[int((self.y+30)/30)][int((self.x+7)/30)] == 1 or active_map[int((self.y+30)/30)][int((self.x+23)/30)] == 1:
            #print("touche")
            self.chute = 0
        ...
        if active_map[int((self.y+30)/30)][int((self.x+20)/30)] == 2:
            self.chute = 0
        ...
        #move
        if (keys[py.K_z] or keys[py.K_UP]) and (active_map[int((self.y+30)/30)][int((self.x+7)/30)] == 1 or active_map[int((self.y+30)/30)][int((self.x+23)/30)] == 1):
            self.chute = -2.1
        if (keys[py.K_z] or keys[py.K_UP]) and active_map[int((self.y+30)/30)][int((self.x+15)/30)] == 2 and not(active_map[int((self.y)/30)][int((self.x+7)/30)] == 1 or active_map[int((self.y)/30)][int((self.x+23)/30)] == 1):
            self.chute = -1.5
        
        if (keys[py.K_s] or keys[py.K_DOWN]) and active_map[int((self.y+30)/30)][int((self.x+15)/30)] == 2:
            self.chute = 1.5
        
        if (keys[py.K_q] or keys[py.K_LEFT]) and (active_map[int((self.y)/30)][int((self.x+5)/30)] != 1 and active_map[int((self.y+27)/30)][int((self.x+5)/30)] != 1):
            self.x -= 1.5
            self.direction = 0
        
        if (keys[py.K_d] or keys[py.K_RIGHT]) and (active_map[int((self.y)/30)][int((self.x+25)/30)] != 1 and active_map[int((self.y+29)/30)][int((self.x+25)/30)] != 1):
            self.x += 1.5
            self.direction = 1
        
        if active_map[int((self.y)/30)][int((self.x+15)/30)] == 1:
            self.chute = 0.3
        ...
        if self.bomb != []:
            try:#la bombe peut aller trop bas à cause de la vitesse et passer sous la map
                if active_map[int((self.bomb[1]+5)/30)][int((self.bomb[0]+2.5)/30)] == 1:
                    while active_map[int((self.bomb[1])/30)][int((self.bomb[0]+2.5)/30)] == 1 and self.bomb[3] > 0:
                        self.bomb[1] -= 30
                ...
                self.bomb[3] = -0.75 * self.bomb[3]
                self.bomb[1] = 565
            if active_map[int((self.bomb[1])/30)][int((self.bomb[0]+2.5)/30)] == 1:
                self.bomb[3] = -0.75 * self.bomb[3]
                self.bomb[1] = (int(self.bomb[1] / 30)) * 30 + 35
            try:#la bombe peut aller trop bas à cause de la vitesse et passer sous la map
                if active_map[int((self.bomb[1]+2.5)/30)][int((self.bomb[0])/30)] == 1:
                    self.bomb[2] = -0.75 * self.bomb[2]
                    self.bomb[0] = (int(self.bomb[0]/30))*30+30
            except:
                self.bomb[3] = -0.75 * self.bomb[3]
                self.bomb[1] = 565
            if active_map[int((self.bomb[1]+2.5)/30)][int((self.bomb[0]+5)/30)] == 1:
                self.bomb[2] = -0.75 * self.bomb[2]
            ...
            self.bomb_reload = round(self.bomb_reload + 0.005, 3)
        else:
            self.bomb_reload = 1
            ...
            if mob.reload >= 1 and abs(mob.y-player.y) <= 20:
                if mob.x <= player.x:
                    ...
                    for a in range(int(mob.x/30), int(player.x/30)):
                        ...
                        if active_map[int(mob.y/30)][a] == 1:
                            fire = False
                else:
                    direction = 0
    ...
    #for a in range(len(bullets)):
        if bullets[a][3] == "player":
            ...
            if bullets[a][0] + 2 >= player.x and bullets[a][0] + 2 <= player.x + 30 and bullets[a][1] >= player.y and bullets[a][1] <= player.y + 30:
                ...
                game = False
        try:
            if active_map[int(bullets[a][1]/30)][int(bullets[a][0]/30)] == 1:
                bullets.pop(a)
        except:
            pass
...
def level_up(a):
    ...
    while continuer:
        ...
        py.display.flip()
    global active_map
    active_map = n_map(levels[a])
            
#####################################################chargement images#####################################################
py.init()
fen = py.display.set_mode((1200, 600))

intro = py.image.load("images/intro.png")
intros = [py.image.load("images/texte_intro_01.png"),
          py.image.load("images/texte_intro_02.png")
          ]
...
ladder.convert()
door = py.image.load("images/door.png")
door.convert()
...
skin_alien = [[py.image.load("images/alien_0.png"),
              py.image.load("images/alien_1.png"),
              py.image.load("images/alien_2.png")]]
...
levels = [["maps/map_01.csv", 0], ["maps/map_02.csv", 15], ["maps/map_03.csv", 20], ["maps/map_03.csv", 30]]
active_map = n_map(levels[0])
...
while level < len(levels):
    ...
    for x in range(len(start_aliens)):
        Alien.aliens.append(Alien(start_aliens[x][0], start_aliens[x][1]))
    main(active_map)
    if player.finish_coo[0] <= player.x + 15 and player.finish_coo[0] + 15 >= player.x and player.finish_coo[1] <= player.y + 15 and player.finish_coo[1] + 15 >= player.y and len(Alien.aliens) == 0:
        level += 1
        if level <len(levels):
